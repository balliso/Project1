...
pums_query <- function(year = 2022, numeric_variables = c("AGEP", "PWGTP"), categorical_variables = c("SEX"), geography = "state", state = "03", filter = NULL){
  ...
  variables <- paste(c(numeric_variables, categorical_variables))
  url_result <- paste0("https://api.census.gov/data/", year, "/acs/acs1/pums?", "variables=", variables, "&geography=", geography, "&state=", state)
}
```

...
```{r}
pums_multi_year <- function(year = 2022, numeric_variables = c("AGEP", "PWGTP"), categorical_variables = c("SEX"), geography = "state", state = "03", filter = NULL){
  years <- as.integer(years)
  all_data <- list()
  for(yr in years){
    test_tibble <- pums_query(
      year = yr, numberic_variables = numberic_variables, categorical_variables = categorical_variables, geography = geography, state = state, filter = filter
    )
    test_tibble$year <- yr
    all_tibbles[[as.character(yr)]] <- test_tibble
  }
  combined_tibble <- dplyr::bind_rows(all_tibbles)

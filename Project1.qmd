...
#the following code is used to turn a given categorical variable into a factor with appropriate levels. I saved each variable as a list of it's items, then saved all of those variables in a list
...
Now, we need to call the function to create a tibble to work with in a later section.
...
summary <- census |>
  reframe(
    ...
    summary |>
      select(-ends_with("_sample_mean_sample_sd"))
      cat_counts <- census|>
  pivot_longer(
    ...
    reframe(weighted_count = sum(PWGTP), .groups = "drop")
